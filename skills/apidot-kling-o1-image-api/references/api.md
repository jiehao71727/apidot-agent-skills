# APIDot Kling O1 Image Reference

This reference gives agents a safer, non-executable summary for APIDot Kling O1 Image integration work. Use it to choose the right documentation path and request mode before opening the live APIDot docs or general examples.

It contains no shell commands, no runnable request examples, no bundled clients, and no stored credentials.

## Source Of Truth

- Model page: https://apidot.ai/models/kling-o1-image
- API docs: https://apidot.ai/docs/kling-o1-image
...
Kling O1 Image on APIDot is for image editing and prompt-guided image workflows where the user wants a practical Kling image model for visual edits, variations, product visuals, campaign assets, or reference-guided output.

Do not assume every image request mode is supported by every Kling O1 Image model entry. Check the APIDot Kling O1 Image docs before preparing a payload.

## Request Planning

Before choosing a request shape, identify:

- Whether the user wants image editing, prompt-only generation, image variation, product visuals, ad creative, or reference-guided generation.
- Whether source image URLs are involved, and whether the selected Kling O1 Image mode supports them.
- Desired aspect ratio, resolution, output format, and output count, if the current APIDot docs expose those controls.
...
APIDot Kling O1 Image jobs follow the shared APIDot async generation flow:
...
- Do not copy fields from another image model family unless the APIDot docs show the same field for Kling O1 Image.
