# APIDot Seedream 4 Reference

This reference gives agents a safer, non-executable summary for APIDot Seedream 4 integration work. Use it to choose the right documentation path and request mode before opening the live APIDot docs or general examples.

It contains no shell commands, no runnable request examples, no bundled clients, and no stored credentials.

## Source Of Truth

- Model page: https://apidot.ai/models/seedream-4
- API docs: https://apidot.ai/docs/seedream-4
...
Seedream 4 on APIDot is for image generation and editing workflows that need prompt-driven visuals, reference image control, product scenes, marketing assets, or higher-resolution image planning when supported by the selected request mode.

Do not assume every image request mode is supported by every Seedream 4 model entry. Check the APIDot Seedream 4 docs before preparing a payload.
...
- Whether source image URLs are involved, and whether the selected Seedream 4 model entry supports reference images.
...
APIDot Seedream 4 jobs follow the shared APIDot async generation flow:
...
- Do not copy fields from another image model family unless the APIDot docs show the same field for Seedream 4.
