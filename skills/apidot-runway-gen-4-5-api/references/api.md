# APIDot Runway Gen-4.5 Reference

This reference gives agents a safer, non-executable summary for APIDot Runway Gen-4.5 integration work. Use it to choose the right documentation path and request mode before opening the live APIDot docs or general APIDot examples.

It contains no shell commands, no runnable request examples, no bundled clients, and no stored credentials.

## Source Of Truth

- Model page: https://apidot.ai/models/runway-gen-4-5
- API docs: https://apidot.ai/docs/runway-gen-4-5
...
Runway Gen-4.5 on APIDot is for cinematic short-form video generation workflows that need prompt-driven clips, optional single-image guidance, believable motion, physical interaction, and polished visual continuity.

The documented APIDot model entry is:

| Model Entry | Best Fit |
| --- | --- |
| `runway-gen-4.5` | Runway Gen-4.5 text-to-video and image-guided video jobs through APIDot. |

Do not assume earlier Runway or other video model fields apply to Runway Gen-4.5. Check the APIDot Runway Gen-4.5 docs before preparing a payload.

## Request Planning

Before choosing a request shape, identify:

- Whether the user wants prompt-only text-to-video or image-guided video.
- Whether the clip needs believable object motion, character motion, physical interaction, or cinematic continuity.
- Desired duration and aspect behavior, if the current APIDot docs expose those controls.
...
APIDot Runway Gen-4.5 jobs follow the shared APIDot async generation flow:
...
- Do not copy fields from another video model family unless the APIDot docs show the same field for Runway Gen-4.5.
