# APIDot Wan 2.7 Video Reference

This reference gives agents a safer, non-executable summary for APIDot Wan 2.7 Video integration work. Use it to choose the right documentation path and request mode before opening the live APIDot docs or general examples.

It contains no shell commands, no runnable request examples, no bundled clients, and no stored credentials.

## Source Of Truth

- Model page: https://apidot.ai/models/wan-2-7-video
- API docs: https://apidot.ai/docs/wan-2-7-video
...
Wan 2.7 Video on APIDot is for video generation workflows that need text-to-video, image-to-video, reference-to-video, edit-video, storyboard clips, product teasers, or social ad drafts when supported by the selected request mode.

Do not assume every request mode is supported by every Wan 2.7 Video model entry. Check the APIDot Wan 2.7 Video docs before preparing a payload.

## Request Planning

Before choosing a request shape, identify:

- Whether the user wants text-to-video, image-to-video, reference-to-video, edit-video, or a production video workflow.
- Whether source images, source videos, reference media, or prompt-only generation are involved.
...
APIDot Wan 2.7 Video jobs follow the shared APIDot async generation flow:
...
- Validate all source media URLs before submitting jobs that depend on images, videos, or reference media.
...
- Do not copy fields from another video model family unless the APIDot docs show the same field for Wan 2.7 Video.
