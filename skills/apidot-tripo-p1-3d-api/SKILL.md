...
This skill is for routing Tripo P1 3D questions to the right APIDot docs, examples, reference notes, and async integration pattern. It is documentation-only: it includes no scripts, makes no network requests, and does not store credentials.
...
Use `references/api.md` for a local, non-executable summary of Tripo P1 model routing, request planning, and async workflow notes.
