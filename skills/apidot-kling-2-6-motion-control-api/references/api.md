# APIDot Kling 2.6 Motion Control Reference

This reference gives agents a safer, non-executable summary for APIDot Kling 2.6 Motion Control integration work. Use it to choose the right documentation path and request mode before opening the live APIDot docs or general examples.

It contains no shell commands, no runnable request examples, no bundled clients, and no stored credentials.

## Source Of Truth

- Model page: https://apidot.ai/models/kling-2-6-motion-control
- API docs: https://apidot.ai/docs/kling-2-6-motion-control
...
Kling 2.6 Motion Control on APIDot is for motion-transfer video workflows that use a character image and a reference motion video to create controllable character animation, dance transfer, action transfer, storyboard previews, or ad creative when supported by the selected request mode.

Documented Kling 2.6 Motion Control model entries include:

| Model Entry | Best Fit |
| --- | --- |
| `kling-2.6-motion-control` | Motion transfer from a reference video onto a character image for controllable animation workflows. |

Do not assume every media input, duration, or resolution option is supported. Check the APIDot Kling 2.6 Motion Control docs before preparing a payload.
...
APIDot Kling 2.6 Motion Control jobs follow the shared APIDot async generation flow:
...
- Do not copy fields from another Kling or video model family unless the APIDot docs show the same field for Kling 2.6 Motion Control.
