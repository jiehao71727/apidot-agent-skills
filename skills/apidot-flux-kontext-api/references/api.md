# APIDot Flux Kontext Reference

This reference gives agents a safer, non-executable summary for APIDot Flux Kontext integration work. Use it to choose the right documentation path and request mode before opening the live APIDot docs or general examples.

It contains no shell commands, no runnable request examples, no bundled clients, and no stored credentials.

## Source Of Truth

- Model page: https://apidot.ai/models/flux-kontext
- API docs: https://apidot.ai/docs/flux-kontext
...
Flux Kontext on APIDot is for context-aware image generation and editing workflows that need character consistency, local edits, typography-aware updates, product visuals, campaign concepts, or design exploration when supported by the selected request mode.

Documented Flux Kontext model entries may include Pro or Max variants. Check the live APIDot docs before choosing a variant or preparing a payload.

## Request Planning

Before choosing a request shape, identify:

- Whether the user wants prompt-only generation, single-image editing, character consistency, local edits, typography-aware updates, product visuals, or design concept imagery.
- Whether source image URLs are involved, and whether the selected Flux Kontext mode supports the requested reference behavior.
...
APIDot Flux Kontext jobs follow the shared APIDot async generation flow:
...
- Do not copy fields from another image model family unless the APIDot docs show the same field for Flux Kontext.
