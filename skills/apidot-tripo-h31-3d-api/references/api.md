# APIDot Tripo H3.1 Reference

This reference gives agents a safer, non-executable summary for APIDot Tripo H3.1 3D integration work. Use it to choose the right documentation path and request mode before opening the live APIDot docs or general examples.

It contains no shell commands, no runnable request examples, no bundled clients, and no stored credentials.

## Source Of Truth

- Model page: https://apidot.ai/models/tripo-h31-3d
- 3D models: https://apidot.ai/models/3d
- API docs: https://apidot.ai/docs/tripo-h31-3d
...
Tripo H3.1 on APIDot is for 3D asset generation workflows that need text-to-3D, image-to-3D, multiview-to-3D, character or object assets, and production async handling when supported by the selected request mode.

Do not assume every 3D request mode is supported by every Tripo H3.1 model entry. Check the APIDot Tripo H3.1 docs before preparing a payload.

## Request Planning

Before choosing a request shape, identify:

- Whether the user wants text-to-3D, image-to-3D, multiview-to-3D, or a generated asset pipeline.
- Whether source images, multiview references, or prompt-only generation are involved.
...
APIDot Tripo H3.1 jobs follow the shared APIDot async generation flow:
...
- Validate all source image URLs before submitting jobs that depend on references or multiview inputs.
...
- Do not copy fields from another 3D model family unless the APIDot docs show the same field for Tripo H3.1.
