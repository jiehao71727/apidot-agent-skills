# APIDot Kling O3 Image Reference

This reference gives agents a safer, non-executable summary for APIDot Kling O3 Image integration work. Use it to choose the right documentation path and request mode before opening the live APIDot docs or general examples.

It contains no shell commands, no runnable request examples, no bundled clients, and no stored credentials.

## Source Of Truth

- Model page: https://apidot.ai/models/kling-o3-image
- API docs: https://apidot.ai/docs/kling-o3-image
...
Kling O3 Image on APIDot is for image generation and editing workflows that need prompt-guided images, reference image control, element-aware changes, product visuals, campaign concepts, or higher-resolution output planning when supported by the selected request mode.

Do not assume every image request mode is supported by every Kling O3 Image model entry. Check the APIDot Kling O3 Image docs before preparing a payload.

## Request Planning

Before choosing a request shape, identify:

- Whether the user wants prompt-only generation, image editing, image-to-image, element-guided changes, product visuals, or design concept imagery.
- Whether source image URLs or element references are involved, and whether the selected Kling O3 Image mode supports them.
- Desired aspect ratio, resolution, and output count, if the current APIDot docs expose those controls.
...
APIDot Kling O3 Image jobs follow the shared APIDot async generation flow:
...
- Do not copy fields from another image model family unless the APIDot docs show the same field for Kling O3 Image.
